...
# Sample Data Creation Script for MoneyMate
# Run this after registering a user and logging in

echo "=========================================="
echo "MoneyMate Sample Data Creator"
echo "=========================================="
echo ""
...
# Income Categories
SALARY=$(curl -s -X POST http://localhost:3000/categories \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{"name":"Salary","type":"INCOME","color":"#059669"}' | jq -r '.id')
echo "✅ Created: Salary"

FREELANCE=$(curl -s -X POST http://localhost:3000/categories \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
