...
describe('UncategorizedTransactionsReport', () => {
  ...
  it('filters transactions by expense type', async () => {
    mockGetUncategorizedTransactions.mockResolvedValue({
      transactions: [
        { id: 'tx-1', transactionDate: '2025-02-15', payeeName: 'Store A', description: '', accountName: 'Chequing', amount: -50 },
        { id: 'tx-2', transactionDate: '2025-02-16', payeeName: 'Employer', description: '', accountName: 'Chequing', amount: 200 },
      ],
      summary: { totalCount: 2, expenseCount: 1, expenseTotal: 50, incomeCount: 1, incomeTotal: 200 },
    });
    render(<UncategorizedTransactionsReport />);
    await waitFor(() => {
  ...
  it('sorts transactions by clicking column headers', async () => {
    mockGetUncategorizedTransactions.mockResolvedValue({
      transactions: [
        { id: 'tx-1', transactionDate: '2025-02-15', payeeName: 'Alpha', description: '', accountName: 'Chequing', amount: -50 },
        { id: 'tx-2', transactionDate: '2025-02-16', payeeName: 'Beta', description: '', accountName: 'Savings', amount: -100 },
      ],
      summary: { totalCount: 2, expenseCount: 2, expenseTotal: 150, incomeCount: 0, incomeTotal: 0 },
    });
    render(<UncategorizedTransactionsReport />);
    await waitFor(() => {
  ...
  it('sorts by amount column', async () => {
    mockGetUncategorizedTransactions.mockResolvedValue({
      transactions: [
        { id: 'tx-1', transactionDate: '2025-02-15', payeeName: 'Store A', description: '', accountName: 'Chequing', amount: -50 },
        { id: 'tx-2', transactionDate: '2025-02-16', payeeName: 'Store B', description: '', accountName: 'Chequing', amount: -100 },
      ],
      summary: { totalCount: 2, expenseCount: 2, expenseTotal: 150, incomeCount: 0, incomeTotal: 0 },
    });
    render(<UncategorizedTransactionsReport />);
  ...
  it('handles clicking a transaction row without error', async () => {
    mockGetUncategorizedTransactions.mockResolvedValue({
      transactions: [
        { id: 'tx-1', transactionDate: '2025-02-15', payeeName: 'Store A', description: 'desc', accountName: 'Chequing', amount: -50 },
      ],
      summary: { totalCount: 1, expenseCount: 1, expenseTotal: 50, incomeCount: 0, incomeTotal: 0 },
    });
    render(<UncategorizedTransactionsReport />);
  ...
  it('renders transaction without payeeName', async () => {
    mockGetUncategorizedTransactions.mockResolvedValue({
      transactions: [
        { id: 'tx-1', transactionDate: '2025-02-15', payeeName: null, description: null, accountName: null, amount: 25 },
      ],
      summary: { totalCount: 1, expenseCount: 0, expenseTotal: 0, incomeCount: 1, incomeTotal: 25 },
    });
    render(<UncategorizedTransactionsReport />);
    await waitFor(() => {
  ...
  it('shows truncation message when more than 100 transactions', async () => {
    ...
    mockGetUncategorizedTransactions.mockResolvedValue({
      transactions,
      summary: { totalCount: 105, expenseCount: 105, expenseTotal: 1050, incomeCount: 0, incomeTotal: 0 },
    });
    render(<UncategorizedTransactionsReport />);
    await waitFor(() => {
      expect(screen.getByText(/Showing first 100 of 105 transactions/)).toBeInTheDocument();
    });
  });

  it('handles API error gracefully', async () => {
    mockGetUncategorizedTransactions.mockRejectedValue(new Error('Network error'));
    render(<UncategorizedTransactionsReport />);
    await waitFor(() => {
      expect(screen.getByText('Total Uncategorized')).toBeInTheDocument();
    });
