...
export class ImportService {
  ...
  async importQifFile(
    ...
    try {
      ...
      // when multiple source accounts map to the same destination)
      const processedAccounts = new Map<string, string>(); // key: accountName, value: accountId
      for (const accMapping of accountsToCreate) {
        ...
        // Check if we already created an account with this name in this import
        ...
        // Check if an account with this name already exists for this user
        // Also check for investment pair naming pattern (e.g., "RRSP - Cash" / "RRSP - Brokerage")
        ...
        if (accountType === AccountType.INVESTMENT) {
          ...
          // Map transfers to the cash account
