...
const incomeCategories: CategoryDef[] = [
  { parent: 'Investment Income', children: ['Capital Gains', 'Interest', 'RESP Grant'] },
  { parent: 'Other Income', children: ['Blogging', 'Business Reimbursement', 'Cashback', 'Consulting', 'Credit Card Reward', 'Employee Stock Option', 'Gifts Received', 'Income Tax Refund', 'Rental Income', 'State & Local Tax Refund', 'Transfer Bonus', 'Tutoring'] },
  { parent: 'Retirement Income', children: ['CPP/QPP Benefits'] },
  { parent: 'Wages & Salary', children: ['Bonus', 'Commission', 'Employer Matching', 'Gross Pay', 'Net Pay', 'Overtime', 'Vacation Pay'] },
];
  ...
  for (const user of users) {
    ...
    for (const catDef of incomeCategories) {
      ...
      if (existingParent.length > 0) {
        ...
        console.log(`  Parent "${catDef.parent}" already exists, marked as income`);
      } else {
