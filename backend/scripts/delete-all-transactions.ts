...
async function main() {
  ...
  try {
    ...
    await dataSource.transaction(async (manager) => {
      ...
      await manager.query("DELETE FROM transaction_splits");
      ...
      await manager.query("DELETE FROM investment_transactions");
      console.log(`Deleted all investment transactions`);

      // Delete all holdings
      await manager.query("DELETE FROM holdings");
      console.log(`Deleted all holdings`);

      // Delete all security prices
      await manager.query("DELETE FROM security_prices");
      console.log(`Deleted all security prices`);

      // Delete all securities
      await manager.query("DELETE FROM securities");
